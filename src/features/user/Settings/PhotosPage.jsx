...
class PhotosPage extends Component {
  ...
  onDrop = (files) => { // grabbing uploaded file content from dropzone and assigning filename
    this.setState({
      files,
  ...
  render() {
    ...
    let filteredPhotos;
    if (photos) { // filter main photo from showing in map as a duplicate photo
        ...
        <Grid>
          ...
          <Grid.Column width={4}>
            ...
            {this.state.files[0] && (
              <div>
                <Image
                  style={{ minHeight: '200px', minWidth: '200px' }} src={this.state.cropResult} />
                <Button.Group>
                  <Button onClick={this.uploadImage} style={{ width: '100px' }} loading={loading} positive icon='check' />
                  <Button onClick={this.cancelCrop} style={{ width: '100px' }} disabled={loading} icon='close' />
                </Button.Group>
              </div>
            )}
          </Grid.Column>
